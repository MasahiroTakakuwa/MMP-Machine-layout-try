  ...
  ngOnInit(): void {
    ...
    timer(0, 15000)  // すぐ実行、その後15秒ごと
      .pipe(
        ...
        switchMap(() => this.machineService.getMachines(6)) // 工場ID 6 (Tierra2)
      )
      .subscribe({
...
onWheel(event: WheelEvent): void {
  ...
  private fetchMachinesOnce(): void {
    this.machineService.getMachines(6)
  ...
  fetchMachines(): void {
    ...
    this.machineService.getMachines(6).pipe(takeUntil(this.destroy$))
    .subscribe({
      next: (data) => {
        this.machines = data.map(element=>{
          if(element.schedule_stop_machine){
  ...
  displayProgResult(){
    ...
    // 工場指定(tierra2:6)
    this.kpiService.getMachiningKPI(6, parts, machine, date).pipe(takeUntil(this.destroy$))
    .subscribe({
  ...
  MachiningProgressResult(){
    ...
    this.kpiService.getMachiningProgress(6,ym,today).pipe(takeUntil(this.destroy$))
        .subscribe({
