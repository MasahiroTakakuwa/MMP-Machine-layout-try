...
import { debounceTime, Subject, timer, Subscription, empty, EMPTY } from 'rxjs';
...
export class MercuryComponent implements OnInit, OnDestroy {
  ...
  constructor( //declare service used in this component
    ...
    // p-tableの初期設定
    // columns = [{ field: 'name', StyleClass:'center-text pre-wrap' }];
    columns = [{ field: 'name'}];
    items = [
    { name: '稼働\n(目標以上)' },
  ...
  ngOnInit(): void {
    ...
    timer(0, 15000)  // すぐ実行、その後15秒ごと
      .pipe(
        ...
        switchMap(() => this.machineService.getMachines(2)) // 工場ID 2 (Mercury)
      )
      .subscribe({
  ...
  private fetchMachinesOnce(): void {
    this.machineService.getMachines(2).pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (data) => this.applyMachines(data),
        error: (err) => console.error('機械API呼び出しエラー:', err),
      });
  ...
  this.ref_dialog.onClose
    .pipe(
      takeUntil(this.destroy$),
      // falsy（閉じただけでデータ無し）を除外
      filter((data: any) => !!data),
      // data.type に応じて API を切替（HTTP は単発だが防御的に takeUntil）
      switchMap((data: any) => {
        if (data.type === 'save-status-machine') {
          return this.machineService.saveStatusMachine(data.value).pipe(
            finalize(() => {/* ここでボタンのローディング解除など可 */})
          );
        } else if (data.type === 'run-machine') {
          return this.machineService.runMachine(data.value).pipe(
            finalize(() => {/* 同上 */})
          );
        } else {
          // 未対応タイプは no-op（空 Observable を返す）
          return EMPTY;
        }
      }),
      takeUntil(this.destroy$) // 念のためチェーン終端にも付けておく
    )
    .subscribe({
      next: () => {
        this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Operation succeeded' });
        this.fetchMachines(); // 成功後に再読込
      },
      error: (error) => {
  ...
  displayProgResult(){
    ...
    // 工場指定(Mercury:2)
    this.kpiService.getMachiningKPI(2, parts, machine, date).pipe(takeUntil(this.destroy$))
    .subscribe({
  ...
  MachiningProgressResult(){
    ...
    this.kpiService.getMachiningProgress(2,ym,today).pipe(takeUntil(this.destroy$))
        .subscribe({
