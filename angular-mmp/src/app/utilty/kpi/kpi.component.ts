...
import { formatK,getWeekendDaysOfCurrentMonth,getWeekdaysThisMonthUntilYesterday } from "../../shared/utils";
...
import { ForgingPlanItem,ForgingProgItem,ForgingProdItems,ForgingResponse,ForgingCurrentResponse,ForgingPastResponse } from "../../interface/forging";
import { MachiningPlanItem,MachiningProgItem,MachiningProgItems,MachiningResponse,MachiningCurrentResponse,MachiningPastResponse,MachiningBaseCTItem } from "../../interface/machining";
...
@Component({
    ...
    standalone:true,
    imports:[ButtonModule,CommonModule,ChartModule,DatePicker,DropdownModule,FormsModule,FluidModule,MessageModule,
...
export class UtilityKpiComponent implements OnInit,OnDestroy{
    ...
    formarprods: ForgingProdItems[] = [];
    formarpasts: ForgingProdItems[] = [];
    // 切削生産計画・進捗データ格納
    machiningplans: MachiningPlanItem[] = [];
    machiningprogs: MachiningProgItem[] = [];
    machiningprods: MachiningProgItems[] = [];
    machiningpasts: MachiningProgItems[] = [];
    ...
    onCheckYearMonth(){
        ...
        // 宣言した年月を取得
        const selyear = this.year;
        const selmonth = this.month;
        const selym = selyear*100+selmonth;
        // 以下KPI参照先変更トライの処理検証
        // 当月
        if(ym === selym){
            // this.displayCharts();
            this.displayChartsCurrent(selym);
        }
        // 過去
        else{
            this.displayChartsPast(selym);
        }
    ...
    // グラフ描画(最新データのみ表示)
    displayChartsCurrent(year_month:number){
        // UIに入力されているデータを格納
        const factory = this.factoryNo | 0;
        const machine = this.machinelistValue?.code;        // 設備
        // const date = getFirstDayOfCurrentMonthInJST();      // 今月1日をstring型で生成
        ...
        if(this.typeNum === 0){
        const planByDay: number[] = new Array(31).fill(0);      //日ごと生産計画数
        this.kpiService.getForgingCurrentKpi(factory, machine, year_month, today).subscribe({
            next: (res: ForgingCurrentResponse) => {
                // --- アクセス方法 ---
                this.formarplans = Array.isArray(res.ForgingCurrentPlan) ? res.ForgingCurrentPlan : [];
                this.formarprods = Array.isArray(res.ForgingCurrentProg) ? res.ForgingCurrentProg : [];
                // 3)グラフ用データを生成
                // 生産計画
                for(let i=0;i<this.formarplans.length;i++){
                    const index = Number(this.formarplans[i].day)-1;
                    planByDay[index] = this.formarplans[i].target_prod;
                    
                }
                // 生産実績
                for(let n=0;n<this.formarprods.length;n++){
                    // テーブル内dayカラムのデータをインデックス番号に変換
                    const day = Number(this.formarprods[n].day)-1; 
                    progByDay[day] = this.formarprods[n].good_prod;       // 良品数
                    // 工程内不良は以下の部分に処理を追加
                    progPerplan[day] = (progByDay[day]/planByDay[day])*100 ;  // 可動率
                    targetPerplan[day] = 85;                                      // 目標可動率
                    inlinedefByDay[day] = (this.formarprods[n].inline_defect/this.formarprods[n].good_prod)*100;      // 工程内不良
                    wastedefByDay[day] = (this.formarprods[n].waste_prod/this.formarprods[n].good_prod)*100;         // 捨て打ち
                    setupdefByDay[day] = (this.formarprods[n].setup_prod/this.formarprods[n].good_prod)*100;         // 段取り
                    // 鍛造の生産実績の累積を格納
                    ProgTotal=Number(ProgTotal)+Number(this.formarprods[n].good_prod);

                }
                for(let m=0;m<this.formarplans.length;m++){
                    // 鍛造の生産計画の累積を格納
                    PlanTotal = Number(PlanTotal)+Number(this.formarplans[m].target_prod);
        ...
        // 切削
            ...
            this.kpiService.getMachiningCurrentKPI(factory, parts, machine, year_month).subscribe({
            next: (res: MachiningCurrentResponse) => {
                // --- アクセス方法 ---
                this.machiningplans = Array.isArray(res.MachiningCurrentPlan) ? res.MachiningCurrentPlan : [];
                this.machiningprods = Array.isArray(res.MachiningCurrentProg) ? res.MachiningCurrentProg : [];
                this.machiningbaseCTs = Array.isArray(res.MachiningBaseCT) ? res.MachiningBaseCT : [];                
                ...
                // 生産実績
                for(let n=0;n<this.machiningprods.length;n++){
                    // テーブル内dayカラムの数値をインデックス番号に変換
                    const day = Number(this.machiningprods[n].day)-1; 
                    progByDay[day] = this.machiningprods[n].good_prod;       // 良品数
                    planByDay[day] = planPerline;     // 生産指示数
                    // 工程内不良は以下の部分に処理を追加
                    progPerplan[day] =(progByDay[day]/baseByDay[day])*100;  // 稼働率
                    targetPerplan[day] = 85;                                    // 目標可動率
                    inlinedefByDay[day] = (this.machiningprods[n].inline_defect/this.machiningprods[n].good_prod)*100;      // 工程内不良
                    visualdefByDay[day] = (this.machiningprods[n].visual_defect/this.machiningprods[n].good_prod)*100;         // 外観不良(捨て打ち)            
                    // 切削稼働日を格納
                    daycount = daycount+1;
                    // 生産実績の累積を格納
                    // Number()で囲わなかった際にstring型と認識されてしまったため、Number()を適用
                    ProgTotal = Number(ProgTotal)+Number(this.machiningprods[n].good_prod);
                    
                }
                // 累計の生産計画数を取得(日当たり計画数×平日日数)
                PlanTotal = Number(planPerline)*weekdays;
    ...
    // 過去データのグラフ描画
    displayChartsPast(yearMonth: number){
        // UIに入力されているデータを格納
        const factory = this.factoryNo | 0;
        const machine = this.machinelistValue?.code;        // 設備
        // const date = getFirstDayOfCurrentMonthInJST();      // 今月1日をstring型で生成
        ...
        if(this.typeNum === 0){
            const planByDay: number[] = new Array(31).fill(0);      //日ごと生産計画数
            this.kpiService.getForgingPastKpi(factory, machine, yearMonth).subscribe({
            next: (res: ForgingPastResponse) => {
                // --- アクセス方法 ---
                this.formarplans = Array.isArray(res.ForgingPastPlan) ? res.ForgingPastPlan : [];
                this.formarpasts = Array.isArray(res.ForgingPastProg) ? res.ForgingPastProg : [];
                ...
                // 生産実績
                for(let n=0;n<this.formarpasts.length;n++){
                    // 日にちをインデックスに変換
                    const index = this.formarpasts[n].day -1;
                    progByDay[index] = this.formarpasts[n].good_prod;       // 良品数
                    // 工程内不良は以下の部分に処理を追加
                    progPerplan[index] = (progByDay[index]/planByDay[index])*100 ;  // 可動率
                    targetPerplan[index] = 85;                                      // 目標可動率
                    inlinedefByDay[index] = (this.formarpasts[n].inline_defect/this.formarpasts[n].good_prod)*100;      // 工程内不良
                    wastedefByDay[index] = (this.formarpasts[n].waste_prod/this.formarpasts[n].good_prod)*100;         // 捨て打ち
                    setupdefByDay[index] = (this.formarpasts[n].setup_prod/this.formarpasts[n].good_prod)*100;         // 段取り
                    
                    // 鍛造の生産実績の累積を格納
                    ProgTotal=Number(ProgTotal)+Number(this.formarpasts[n].good_prod);
                    daycount++;
        ...
        // 切削
            ...
            this.kpiService.getMachiningPastKPI(factory, parts, machine, yearMonth).subscribe({
            next: (res: MachiningPastResponse) => {
                // --- アクセス方法 ---
                this.machiningplans = Array.isArray(res.MachiningPastPlan) ? res.MachiningPastPlan : [];
                this.machiningpasts = Array.isArray(res.MachiningPastProg) ? res.MachiningPastProg : [];
                this.machiningbaseCTs = Array.isArray(res.MachiningBaseCT) ? res.MachiningBaseCT : [];                
                // グラフ用データを生成
                // 生産計画(切削の生産計画は品番ごとのため、1ライン当たりの生産数を算出)
                ...
                // 生産実績
                for(let n=0;n<this.machiningpasts.length;n++){
                    // 日付部分をintに変換
                    const index = this.machiningpasts[n].day -1;
                    progByDay[index] = this.machiningpasts[n].good_prod;       // 良品数
                    planByDay[index] = planPerline;     // 生産指示数
                    // 工程内不良は以下の部分に処理を追加
                    // progPerplan[index] =(progByDay[index]/baseByDay[index])*100;  // 稼働率
                    if(progByDay[index] !== 0){
                        progPerplan[index] = (progByDay[index]/prodByBaseCT)*100;
                        targetPerplan[index] = 85;                                    // 目標可動率
                        inlinedefByDay[index] = (this.machiningpasts[n].inline_defect/this.machiningpasts[n].good_prod)*100;      // 工程内不良
                        visualdefByDay[index] = (this.machiningpasts[n].visual_defect/this.machiningpasts[n].good_prod)*100;      // 外観不良(捨て打ち)
                    }
                    ...
                    // Number()で囲わなかった際にstring型と認識されてしまったため、Number()を適用
                    PlanTotal = Number(PlanTotal+planPerline);
                    ProgTotal = Number(ProgTotal)+Number(this.machiningpasts[n].good_prod);
