  ...
  ngOnInit(): void {
    ...
    timer(0, 15000)  // すぐ実行、その後15秒ごと
      .pipe(
        ...
        switchMap(() => this.machineService.getMachines(1)) // 工場ID 1 (Jupiter)
      )
      .subscribe({
...
onWheel(event: WheelEvent): void {
  ...
  fetchMachines(): void {
    ...
    this.machineService.getMachines(1).pipe(takeUntil(this.destroy$))
    .subscribe({
      next: (data) => {
        this.machines = data.map(element=>{
          if(element.schedule_stop_machine){
  ...
  displayProgResult(){
    ...
    let ProgTotal = 0;
    // 工場指定(Jupiter:1)
    this.kpiService.getForgingKpi(1, parts, machine, date).pipe(takeUntil(this.destroy$))
    .subscribe({
