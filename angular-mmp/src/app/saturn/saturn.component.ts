...
export class SaturnComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    timer(0, 15000)  // すぐ実行、その後15秒ごと
      .pipe(
        ...
        switchMap(() => this.machineService.getMachines(5)) // 工場ID 5 (Saturn)
      )
      .subscribe({
  ...
  private fetchMachinesOnce(): void {
    this.machineService.getMachines(5)
  ...
  fetchMachines(): void {
    ...
    this.machineService.getMachines(5).pipe(takeUntil(this.destroy$))
    .subscribe({
      next: (data) => {
        this.machines = data.map(element=>{
          if(element.schedule_stop_machine){
  ...
  //   // 工場指定(Saturn:5)
  //   // 鍛造
  //   this.kpiService.getForgingKpi(5, parts, machine, date).pipe(takeUntil(this.destroy$))
  //   .subscribe({
  ...
  //   // 工場指定(Saturn:5)
  //   // 切削
  //   this.kpiService.getMachiningKPI(5, parts, machine, date).pipe(takeUntil(this.destroy$))
  //   .subscribe({
  ...
  ForgingProgressResult(){
    ...
    this.kpiService.getProgressForging(5,ym,today).pipe(takeUntil(this.destroy$))
        .subscribe({
  ...
  MachiningProgressResult(){
    ...
    const ym = year*100+month;
    let weekdays = getWeekdaysThisMonthUntilYesterday(now);
    this.kpiService.getMachiningProgress(5,ym,today).pipe(takeUntil(this.destroy$))
        .subscribe({
